...
foreach my $pollo (sort keys %pollos){
	...
	if (-f $pollos{$pollo} and $go){
		...
		$ptask{'job-name'} = $pollo.'_sortIndex';
		...
		# MarkDuplicates
		$ptask{'job-name'} = $pollo.'_markDuplicates';
		...
		$ptask{dependency} = "afterok:$jid";
		my $jid0 = slurmexec(\%ptask);
		push @ljobids, $jid0;
		# BaseRecalibrator
		$ptask{'job-name'} = $pollo.'_baseRecalibrator';
		...
		$ptask{'job-name'} = $pollo.'_applyBQSR';
		...
		$ptask{'job-name'} = $pollo.'_haplotypeCaller';
