...
my %ptask = ('cpus-per-task' => 8, time => '72:0:0', 'mem-per-cpu' => '4G', debug => $test);
...
foreach my $pollo (@pollos){
	...
	if (-d "$wesconf{src_dir}/$pollo" and $go){
		...
		# VerifyBamID (freemix)
		$ptask{'job-name'} = $pollo.'_verifyBamID';
		...
		$job = slurmexec(\%ptask); push @jobs, $job;
		# Sort to coordinate
		$ptask{'job-name'} = $pollo.'_coordinateSortSam';
		...
		unless ($mode eq 'wgs'){
			...
			$ptask{cpus} = 4;
			$ptask{'job-name'} = $pollo.'_collectRawMetrics';
			...
			$mjob = slurmexec(\%ptask); push @mjobs, $mjob;
			$ptask{'job-name'} = $pollo.'_collectWgsMetrics';
			...
			$ptask{dependency} = "afterok:$jid";
			$mjob = slurmexec(\%ptask); push @mjobs, $mjob; 
			$ptask{'job-name'} = $pollo.'_collectPaddedMetrics';
		...
		# VariantEval
		if(exists($ptask{'dependency'})){ delete($ptask{'dependency'}) };
		$ptask{'cpus-per-task'} = 8;
		$ptask{'job-name'} = $pollo.'_variantEval';
		...
		$job = slurmexec(\%ptask); push @jobs, $job;
		# cleanShit
		$ptask{'job-name'} = $pollo.'_closeSubject';
...
unless ($test) {
	my %wtask = ('cpus-per-task' => 1, 'job-name' => 'end_metrics', filename => $slurmdir.'/end.sh', output => $slurmdir.'/end.out', dependency => 'afterok:'.join(',afterok:',@jobs));
	slurmexec(\%wtask);
}
